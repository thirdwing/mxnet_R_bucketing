...
mx.model.train.rnn.buckets <- function(ctx,
  ...
  params.index <-
    as.integer(mx.util.filter.null(lapply(1:length(train.execs[[1]]$ref.grad.arrays), function(k) {
  ...
  for (iteration in begin.round:end.round) {
    ...
    while (train.data$iter.next()) {
      ...
      if (!is.null(kvstore)) {
        # push the gradient
        kvstore$push(params.index,
                     lapply(train.execs, function(texec) {
      ...
      if (update.on.kvstore) {
        ...
        for (i in 1:ndevice) {
          mx.exec.update.arg.arrays(train.execs[[i]], arg.blocks[[i]], skip.null =
                                      TRUE)
        }
      }
      ...
      if (!is.null(metric)) {
        if (ndevice == 1) {
          train.metric <-
            metric$update(
              label = mx.nd.Reshape(slices$label, shape = -1),
              pred = out.preds[[i]][[length(out.preds[[i]])]],
              state = train.metric
            )
        } else{
          for (i in 1:ndevice) {
            train.metric <-
              metric$update(
                label = mx.nd.Reshape(slices[[i]]$label, shape = -1),
                pred = out.preds[[i]][[length(out.preds[[i]])]],
                state = train.metric
              )
          }
    ...
    if (!is.null(epoch.end.callback)) {
      epoch_continue <-
        epoch.end.callback(iteration, 0, environment(), verbose = verbose)
    }
