...
gru.cell <- function(num.hidden,
  ...
  reset.gate <-
    mx.symbol.Activation(split.gates[[2]], act.type = "sigmoid")
  
  htrans.i2h <- mx.symbol.FullyConnected(
    data = indata,
  ...
  htrans.h2h <- mx.symbol.FullyConnected(
    data = h.after.reset,
  ...
  if (is.null(prev.state)) {
    ...
    next.h <-
      prev.state$h + update.gate * (h.trans.active - prev.state$h)
  }
