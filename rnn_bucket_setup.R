...
rnn.unroll <- function(num.rnn.layer,
  ...
  param.cells <- lapply(1:num.rnn.layer, function(i) {
    if (cell.type == "lstm") {
      cell <-
        list(
          i2h.weight = mx.symbol.Variable(paste0("l", i, ".i2h.weight")),
      ...
      cell <-
        list(
          gates.i2h.weight = mx.symbol.Variable(paste0("l", i, ".gates.i2h.weight")),
  ...
  data_mask_array <-
    mx.symbol.stop_gradient(data_mask_array, name = "data.mask.array")
  
  embed <- mx.symbol.Embedding(
    data = data,
  ...
  if (config == "seq-to-one") {
    ...
    last.hidden_expand = lapply(last.hidden, function(i)
      mx.symbol.expand_dims(i, axis = 1))
    concat <-
      mx.symbol.concat(last.hidden_expand, num.args = seq.len, dim = 1)
